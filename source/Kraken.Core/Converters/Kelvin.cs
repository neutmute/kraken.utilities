...
namespace Kraken.Core
{
    ...
    public static class Kelvin<T> // where T : ISerializable // prevents string[] from being Serialized
    {
        ...
        {
            return ToXmlFile(cryo, fileName, new XmlSerializerNamespaces());
        }
        ...
        /// <param name="cryo">T instance to serialize</param>
        /// <param name="fileName">Full file path, including name and extension, eg @"C:\Temp\NewFile.xml"</param>
        /// <param name="namespaces">Include namespaces for the XmlSerializer to use</param>
        /// <returns>true if save was successful, false if an error occured</returns>
        public static bool ToXmlFile(T cryo, string fileName, XmlSerializerNamespaces namespaces)
        {
            try
            {
                XmlSerializer serializerXml = new XmlSerializer(typeof(T));
                System.IO.TextWriter writer = new System.IO.StreamWriter(fileName);
                serializerXml.Serialize(writer, cryo, namespaces);
                writer.Close();
                return true;
            }
